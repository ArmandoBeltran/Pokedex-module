...
class PokemonEvolveWizard(models.TransientModel): 
    ...
    def evolve_confirm(self): 
        for ev_wizd in self:
            if not ev_wizd.sure: 
                raise UserError("You haven't confirmed to evolve: %s"%(ev_wizd.pokemon_id.name))
            if ev_wizd.pokemon_id not in ev_wizd.trainer_id.pokemon_ids: 
                raise UserError("You don't have the next pokemon: %s"%(ev_wizd.pokemon_id.name))
            if not ev_wizd.evolve_to_id: 
                raise UserError("%s not have an evolution available."%(ev_wizd.pokmeon_id.name))
